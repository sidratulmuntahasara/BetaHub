...
function hero() {
  return (
    <Box className='flex flex-col justify-center items-center h-screen gap-6'>
      <Box className='relative h-screen w-11/12 flex flex-col justify-center items-center rounded-t-3xl overflow-hidden border-opacity-80 border-b-0 border-slate-600 border-2 border-solid'>
      <span aria-hidden className='absolute inset-0 scale-x-[3] scale-y-[3] blur-xl before:absolute before:animate-disco before:inset-0 before:aspect-auto before:bg-gradient-conic before:from-pink-300 before:to-indigo-600 before:bg-opacity-50' />
        ...
        <a href='#sec1' className='flex justify-center'><Image src={'/scroller.png'} height={'100'} width={'100'} className='animate-spin' /></a>
     </Box>
    </Box>
  )
}
